...
Seeds: AI tools, learning topics, categories, technologies, and a daily quiz.
...
sys.path.insert(0, "/home/tanmay/Desktop/Dashbord/backend")
...
for t in ai_tools:
    ...
    existing = db.scalar(select(AITool).where(AITool.slug == t["slug"]))
    if not existing:
        db.add(AITool(**t))
        added_tools += 1
db.commit()
print(f"✅ AI Tools: {added_tools} added")
...
topics = [
    ...
    dict(title="TypeScript Utility Types & Type Guards", description="Master Partial, Pick, Omit, Record, and user-defined type guards (is Type) in TypeScript.", difficulty_level="Advanced"),
    # Frontend & Frameworks
    ...
    dict(title="React Performance & Memoization", description="Optimize component re-rendering using useMemo, useCallback, and React.memo for high-performance web apps.", difficulty_level="Advanced"),
    dict(title="Next.js Server Components & App Router", description="Build lightning-fast fullstack web applications with Next.js Server Components and dynamic routing.", difficulty_level="Intermediate"),
    dict(title="State Management with Zustand & Redux", description="Manage global application state cleanly with modern lightweight state stores.", difficulty_level="Intermediate"),
    ...
    dict(title="GraphQL vs REST — Schema & Resolvers", description="Understand GraphQL query language, type schemas, resolvers, and client caching benefits.", difficulty_level="Intermediate"),
    dict(title="Microservices Architecture & Event-Driven Systems", description="Design decoupled, event-driven architectures with message brokers, pub/sub, and eventual consistency.", difficulty_level="Advanced"),
    dict(title="Caching Strategies with Redis", description="Implement cache-aside, write-through, TTL invalidation, and session management with Redis.", difficulty_level="Intermediate"),
    dict(title="WebSockets & Real-Time Communication", description="Build full-duplex, event-driven real-time communication for chats, notifications, and live feeds.", difficulty_level="Intermediate"),
    # Databases & Storage
    ...
    dict(title="PostgreSQL Indexing & Query Optimization", description="Speed up database queries with proper indexing strategies, EXPLAIN ANALYZE, and performance tuning.", difficulty_level="Advanced"),
    dict(title="Database Transactions & ACID Guarantees", description="Understand Atomicity, Consistency, Isolation, Durability, row locks, and transaction rollbacks.", difficulty_level="Intermediate"),
    dict(title="Vector Databases & Similarity Search", description="Store and query high-dimensional embeddings for semantic search and AI applications with pgvector.", difficulty_level="Advanced"),
    ...
    dict(title="Kubernetes Pods, Deployments & Services", description="Orchestrate containerized services with Kubernetes deployments, replica sets, and cluster networking.", difficulty_level="Advanced"),
    dict(title="CI/CD Pipelines with GitHub Actions", description="Automate testing, linting, building, and zero-downtime deployment pipelines.", difficulty_level="Intermediate"),
    dict(title="Git & GitHub — Version Control Workflow", description="Master the Git workflow: branching, merging, rebasing, pull requests, and collaboration on GitHub.", difficulty_level="Beginner"),
    # AI & Machine Learning
    ...
    dict(title="Retrieval-Augmented Generation (RAG) Architecture", description="Connect LLMs to custom knowledge bases using embeddings, vector retrieval, and context injection.", difficulty_level="Intermediate"),
    dict(title="Building Autonomous AI Agents", description="Design ReAct loops, tool calling, memory management, and autonomous multi-step reasoning agents.", difficulty_level="Advanced"),
    # Security & Best Practices
    dict(title="OWASP Top 10 Web Vulnerabilities", description="Protect applications against injection attacks, broken access control, cross-site scripting, and CSRF.", difficulty_level="Intermediate"),
    dict(title="Authentication: JWT, OAuth2 & Sessions", description="Implement secure user authentication, token refresh flows, and OAuth2 social logins.", difficulty_level="Intermediate"),
]

topic_objs = []
added_topics = 0
for t in topics:
    existing = db.scalar(select(LearningTopic).where(LearningTopic.title == t["title"]))
    if not existing:
        obj = LearningTopic(**t)
        db.add(obj)
        db.flush()
...
db.commit()
print(f"✅ Learning Topics: {added_topics} added")
...
for t in techs:
    existing = db.scalar(select(Technology).where(Technology.name == t["name"]))
    if not existing:
        db.add(Technology(**t))
db.commit()
print(f"✅ Technologies: {len(techs)} ensured")

# ─── Daily Quiz ──────────────────────────────────────────────────────────────
topic = topic_objs[0]  # Python Variables topic
...
if not existing_quiz:
    ...
    questions = [
        ...
        dict(question_text="Which operator is used for integer division in Python?", options=["/", "//", "%", "**"], correct_option_index=1),
        dict(question_text="What is the output of: bool(0)?", options=["True", "False", "None", "Error"], correct_option_index=1),
        dict(question_text="How do you add an element to a Python list?", options=[".add()", ".push()", ".append()", ".insert(0)"], correct_option_index=2),
        dict(question_text="What is f-string used for in Python?", options=["File reading", "String formatting", "Function definitions", "Float operations"], correct_option_index=1),
        dict(question_text="Which of these is NOT a valid Python data type?", options=["str", "float", "char", "bool"], correct_option_index=2),
    ]

    for q in questions:
        db.add(DailyQuizQuestion(id=uuid4(), quiz_id=quiz.id, **q))
    db.commit()
    print(f"✅ Daily Quiz: created with {len(questions)} questions")
else:
    print("✅ Daily Quiz: already exists")

# ─── Daily Learning Content ──────────────────────────────────────────────────
topic_for_content = topic_objs[0]
existing_content = db.scalar(select(DailyLearningContent))
if not existing_content:
    content = DailyLearningContent(
        id=uuid4(),
        topic_id=topic_for_content.id,
        content_text=(
    ...
    db.add(content)
    db.commit()
    print("✅ Daily Learning Content: created")
else:
    print("✅ Daily Learning Content: already exists")

db.close()
print("\n🎉 Seed complete! Restart the backend server and refresh the browser.")
